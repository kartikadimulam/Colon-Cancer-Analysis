...
```
Similar to our data subsets where etype = 1 and etype = 2, adding nodes as the next covariate after rx gave us the lowest AIC, 12686.50. Differ and node4 followed with the next lowest AIC scores of 12815.31 and 13019.81 respectively. 
...
```
Adding surg gives us an AIC of 12294.01.
...
```
Looking at the BIC, we can see that the previous model without adhere has a lower BIC of 12327.44. Our final model for the combined dataset includes the covariates rx, nodes, differ, extent, obstruct, and surg. 

```{r, include = FALSE}
anova(full.rndeo.surg)
```

```{r}
cox.zph(full.rndeo.surg)
```
Using a cox ZPH test, we can see that differ and obstruct violate the proportionality assumption because their p-values are less than 0.05 (7.1e-06 and 0.0022) meaning they are time dependent covariates that we need to take care of through strata.  

```{r}
comb.strat <- coxph(Surv(start, stop, status) ~ rx + nodes + strata(differ) + extent + strata(obstruct) + surg, data = colon)
cox.zph(comb.strat)
```
We run cox ZPH again on the new model that stratifies differ and obstruct and see that all the p-values are greater than 0.05 so we do not violate the proportionality assumption. 
