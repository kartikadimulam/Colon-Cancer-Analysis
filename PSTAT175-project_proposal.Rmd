...
```
Similar to our data subsets where etype = 1 and etype = 2, adding nodes as the next covariate after rx gave us the lowest AIC, 11443.35. Node4 follows with the next lowest AIC score of 11743.73. We will not use node4 as a covariate since it is essentially measuring the same quantity as the nodes covariate. We do not want to use highly correlated covariates in our cox PH model. 
...
Adding differ gives us the lowest AIC of 11128.39 
...
AIC(full.rnd.sex, full.rnd.age, full.rnd.obstruct, full.rnd.perfor,
    ...
    full.rnd.surg)
```
Adding extent as our next covariate gives us an AIC of 11091.87 
...
Adding surg gives us the lowest AIC of 11083.24 
...
Adding obstruct gives us an AIC of 11079.07.
This change is minimal, so we will verify it using BIC.

```{r, include. = FALSE}
BIC(full.rnde.surg, full.rndes.obstruct)
```
By using BIC, which penalizes the number of covariates more strictly than AIC, we can see that the model with surg is a better model as it gives a BIC of 11111.89 as opposed to 11112.5 for the model that adds obstruct as another covariate. We will stop adding covariates here. 


```{r, include = FALSE}
anova(full.rnde.surg)
```

We verify using the likelihood ratio test that all of our covariates have a significant effect on time until recurrence or death. 
...
ggsurvplot(colon.fit.differ, colon, legend.labs=c('Well','Moderate', 'Poor'), fun='cloglog') + 
  labs(x='Days until Recurrence')
```
Using a cox ZPH test and a log-log plot, we can see that although differ was previously determined to have a significant effect, it violates the proportional hazards assumption. To fix this, we will construct a stratified model that stratifies on the 3 levels of tumor differentiation.   

```{r}
comb.strat <- coxph(Surv(start, stop, status) ~ rx + nodes + strata(differ) + extent + surg + strata(etype), data = colon)

cox.zph(comb.strat)
AIC(comb.strat)
```
We verify again with the cox ZPH test that none of our covariates are in violation of the proportional hazards assumption after stratifying on tumor differentiation. In addition, we have massively reduced our AIC score to 9728.77 by fixing this violation. 
